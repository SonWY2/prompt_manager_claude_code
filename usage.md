...
[description]
개발자가 프로젝트의 구조와 동작을 빠르게 이해할 수 있도 각 모듈의 사용법을 문서화합니다.
프로젝트의 핵심 가치를 높이기 위한 모든 기능을 빠르게 활용할 수 있도 API 설계를 제공합니다.
...
├── data/          # TinyDB 데이터 파일
├── src/
│   ├── core/       # 비즈니스 로직 (Main Logic + Plugin)
...
- **main_window.py**: 메인 윈도우 (3단 레이아웃, Modern Dark Mode)
- **widgets/**: 각 UI 위젯

**Core Layer**:
- **task_manager.py**: 태스크 CRUD 관리
- **version_manager.py**: 버전 관리 (단순 스냅샷)
- **template_engine.py**: 템플릿 엔진 ({{변수명}} 형식)
- **llm_service.py**: LLM API 통합
- **provider_manager.py**: LLM Provider 관리
- **plugin_interface.py**: 플러그인 인터페이스

**Data Layer**:
- **models.py**: 데이터 모델 (Task, Prompt, Version, ExecutionRecord, Provider)
- **database.py**: TinyDB 데이터베이스
- **repository.py**: Repository 패턴 (BaseRepository, 구현체들)

**Utils Layer**:
- **string_utils.py**: 템플릿 변수 파싱
- **id_generator.py**: 고유 ID 생성
- **config.py**: 상수 및 설정 (Magic Number 방지)
